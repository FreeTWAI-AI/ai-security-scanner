...
依**開始新掃描 → 掃描設定 → 開始掃描 → 掃描進度 → 掃描結果**前進。在掃描結果按**保存或分享報告**進入**分享結果**，再按**儲存「HTML 報告（建議）」**。詳見[取得 HTML 報告的逐步操作](docs/getting-started.zh-TW.md#從開始新掃描到-html-報告)，也包含如何從**我的掃描**回到原本的專案。
