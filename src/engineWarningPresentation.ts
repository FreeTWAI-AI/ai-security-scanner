...
export const recognizedEngineWarningZhTW = (warning: string): string | undefined => {
  ...
  const rules: ReadonlyArray<readonly [RegExp, (...values: string[]) => string]> = [
    ...
    [/^garak output was not its supported JSONL run report; the raw artifact was retained, and the scan should be retried with the supported pinned JSONL output$/u, () => `garak 輸出不是支援的 JSONL 執行報告；原始成品已保留，請使用支援的固定 JSONL 輸出重試`],
